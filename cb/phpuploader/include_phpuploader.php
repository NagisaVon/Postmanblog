...
function PhpUploader_MoveUploadedFile($_file,$_line,$src,$dst)
{
	...
	//$er=error_reporting(0);iconv($PhpUploader_InternalEncoding,$PhpUploader_FSEncoding,$dst)
...
class PhpUploader
{
	...
	/// To identify the article or problem id;
	public $FromID;
	/// To identify the article or problem id;
	public $FromUserID;
	/// To identify that this uploaded file is attached to an article or a problem;
	public $FromClass;
	function Return_ids () {
		global $FromID, $FromUserID, $FromClass;
		return array(
			'id'     => $FromID,
			'userid' => $FromUserID,
			'class'  => $FromClass
	...
	public $CancelAllMsg="清空上传队列";
	
	/// Gets or sets the "Cancel upload" message.
	public $CancelUploadMsg="取消上传";
	...
	function PhpUploader()
	{
		...
		$this->ResourceDirectory="phpuploader/resources";
		$this->ResourceHandler="phpuploader/ajaxuploaderresource.php";
		$this->UploadUrl="phpuploader/ajaxuploaderhandler.php";
	...
	function Render()
	{
		...
		{
			echo("对不起，发生错误！错误代码：" . $x->getMessage());
			exit(200);
		}
	} 
	...
	function GetString()
	{
		...
		if($this->SaveDirectory)
		{
			if(!$this->AllowedFileExtensions)
			{
				...
				$code.="When using SaveDirectory property, you must  AllowedFileExtensions for security purpose.";
				$code.="</div>";
			}
		}
		...
		{
			$code.="<input id='" . $this->Name . "Button' type='file' class='btn btn-default'>" . $this->InsertText . "</button>";
		}
		...
		$code.=$this->_GenerateAttribute("FromID");
		$code.=$this->_GenerateAttribute("FromClass");
	...
	function GetValidatingFile()
	{
		if(!$this->_filevalidating)
		{
			...
			if(!$this->_filevalidating)
			{
				...
				throw(new Exception("无法找到文件 " . $this->_fileguid));
			}
		}
	...
	function _InternalGetFile($guid,$checkValidated)
	{
		...
		if( $checkValidated && substr(PhpUploader_GetBaseName($arr[0]),0,10)!="persisted.")
			throw(new Exception("This file can not be !"));
	...
	function SaveSecuritySettings()
	{
		...
		$this->_SaveSecuritySetting("FromID");
		$this->_SaveSecuritySetting("FromClass");
	...
	function LoadSecuritySettings()
	{
		...
		$this->_LaveSecuritySetting("FromID");
		$this->_LaveSecuritySetting("FromClass");
